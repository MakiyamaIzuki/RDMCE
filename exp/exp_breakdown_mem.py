...
# 手动指定方法数组，可根据实际情况修改
METHODS = ['RDMCE', 'mce-gpu-PX']  # 请替换为实际方法名


def main():
    # 读取 CSV 文件
    file_path = 'res_mem.csv'
    ...
    for i, method in enumerate(METHODS):
        if method in df.columns:
            ...
            # 将MiB转换为GB (1 GB = 1024 MiB)
            y_values = df.loc[valid_mask, method] / 1024
    ...
    plt.legend(loc='upper left', fontsize=12, bbox_to_anchor=(0, 0.95))
    
    plt.axhline(y=24, color='red', linestyle='--', linewidth=2)
    # plt.text(0.5, 24.5, '4090 GPU Capacity (24GB)', ha='center', va='bottom', color='red', fontweight='bold')
